...
| `RI_HTTP_PROXY` | 远程链接检查时使用的可选 HTTP 代理。 | `http://proxy.example:8080` |
...
下面的命令示例假设 `php` 已加入 `PATH`，并且必要扩展已经启用。如果你的 PHP 不会默认加载扩展，请按自己的环境增加对应的 `-d extension=...` 参数。
...
容器默认启动时会自动索引。若想手动控制索引，可设置 `RI_AUTO_INDEX_ON_START=false`，再用 `docker run --rm --env-file .env -v ${PWD}/images:/app/images -v ${PWD}/.runtime:/app/.runtime ghcr.io/OWNER/REPOSITORY:v1.0.0 php bin/console.php index` 单独执行。
